...
class TestMontyHallTableRandomChooseAvailable(unittest.TestCase):
    ...
    # door 'b' is 1. (He cannot pick door 'c'.)
...
class TestMontyHallTableRandomChooseBIfPossible(unittest.TestCase):
    # The hypotheses, 'a', 'b', and 'c', model the situation in which the car is behind door named by the hypothesis.
    # For example, hypothesis 'a' means that car is *actually* behind door 'a'.
    #
    # To calculate the likelihoods, given that you have picked door 'a', we reason as follows:
    #
    # If the car is behind door 'a', Monty Hall can safely pick either door 'b' or 'c'; therefore,
    # he picks 'b' with probability 1. If the car is behind door 'b', the likelihood of Monty Hall picking door 'b' is
    # 0. Finally, if the car is behind door 'c', the likelihood of Monty Hall picking door 'b' is also 1. (He picks
    # door 'b' if he can.)

    table_data = {'a': think_bayes.Table.Row(fractions.Fraction(1, 3), 1),
                  'b': think_bayes.Table.Row(fractions.Fraction(1, 3), 0),
                  'c': think_bayes.Table.Row(fractions.Fraction(1, 3), 1)}
    ...
    def test_monty_hall_stay_with_a(self):
        ...
        self.assertEqual(fractions.Fraction(1, 2), sut.posterior('a'))
    ...
    def test_monty_hall_change_to_c(self):
        ...
        self.assertEqual(fractions.Fraction(1, 2), sut.posterior('c'))
